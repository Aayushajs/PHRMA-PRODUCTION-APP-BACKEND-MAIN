...
r.post('/signup', uploadImage.single('profileImage'), validateRequest({ body: signupSchema }), UserService.signup);
r.post('/login', validateRequest({ body: loginSchema }), UserService.login);
...
r.post('/forgot-password', validateRequest({ body: forgotPasswordSchema }), UserService.forgotPassword);
r.post('/verify-otp', validateRequest({ body: verifyOtpSchema }), UserService.verifyOtp);
r.post('/reset-password', userMiddleware, validateRequest({ body: resetPasswordSchema }), UserService.ResetPassword);
r.post('/google-login', validateRequest({ body: googleLoginSchema }), UserService.googleAuthLogin);
