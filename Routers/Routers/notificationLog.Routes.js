...
router.patch('/mark-multiple-read', authenticatedUserMiddleware, async (req, res) => {
    try {
...
// ============================================================================
router.post('/register-token', authenticatedUserMiddleware, async (req, res) => {
    try {
        const userId = req.user?._id;
        ...
        const user = await UserModel.findByIdAndUpdate(userId, { fcmToken: token.trim() }, { new: true });
        if (!user) {
...
// ============================================================================
router.post('/send-test', authenticatedUserMiddleware, async (req, res) => {
    try {
        const userId = req.user?._id;
        ...
        const result = await NotificationService.sendNotificationWithLog(userId, fcmToken, title || 'Test Notification', body || 'This is a test notification', {
            type: type,
            payload: {
                source: 'api_test',
