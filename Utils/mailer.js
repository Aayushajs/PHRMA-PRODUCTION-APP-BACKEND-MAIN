...
const sendViaSendGrid = async (to, template) => {
    if (!sendGridReady)
        return false;
    try {
        await sgMail.send({
...
const sendViaMailjet = async (to, template) => {
    if (!mailjetReady)
        return false;
    try {
...
const sendViaGmail = async (to, template) => {
    if (!gmailTransporter)
        return false;
    try {
...
export const sendEmail = async (to, type = 'otp', data) => {
