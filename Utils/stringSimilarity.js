 ...
 */
export function extractPackSize(packStr) {
    if (!packStr)
        return 1;
 ...
 */
export function calculateCompositeSimilarity(localName, localFormula, localCompany, remoteName, remoteFormula, remoteCompany) {
    const nameSim = stringSimilarity(normalizeProductName(localName), normalizeProductName(remoteName));
    ...
    const companySim = localCompany && remoteCompany
        ? stringSimilarity(normalizeProductName(localCompany), normalizeProductName(remoteCompany))
        : 0;
