...
export default class GSTServices {
    static createGST = catchAsyncErrors(async (req, res, next) => {
        const { gstName, gstRate, gstDescription, cgstRate, sgstRate, igstRate, isActive, applicableFrom, applicableTo, } = req.body;
        ...
        if (existingGST) {
            return next(new ApiError(409, "GST with same name or rate already exists"));
        }
        const newGSTData = {
        ...
        return handleResponse(req, res, 201, "GST created successfully", newGST);
    });
    static getAllGSTs = catchAsyncErrors(async (req, res, next) => {
        const gstRecords = await gstModel.find();
        if (gstRecords.length === 0) {
            return next(new ApiError(404, "No GST records found"));
        }
        return handleResponse(req, res, 200, "GST records retrieved successfully", gstRecords);
    });
    static updateGST = catchAsyncErrors(async (req, res, next) => {
        ...
        const updateGSTData = await gstModel.findByIdAndUpdate(gstId, { ...updateData, updatedBy: req.user?._id, updatedAt: new Date() }, { new: true });
        if (!updateGSTData) {
            return next(new ApiError(500, "Failed to update GST record"));
        }
        return handleResponse(req, res, 200, "GST record updated successfully", updateGSTData);
    });
    static deleteGST = catchAsyncErrors(async (req, res, next) => {
        const gstId = req.params.gstId;
        const existingGST = await gstModel.findById(gstId);
        if (!existingGST) {
            return next(new ApiError(404, "GST record not found"));
        ...
        const updateItems = await gstModel.updateMany({ itemGST: gstId }, { $unset: { gstId: "" } });
        console.log(`Cleaned up ${updateItems.modifiedCount} item(s) that referenced deleted GST.`);
        return handleResponse(req, res, 200, "GST record deleted successfully", null);
    });
}
