...
export default class ItemServices {
    ...
    static createItem = catchAsyncErrors(async (req, res, next) => {
        const { itemName, itemInitialPrice, itemDescription, itemCategory, itemMfgDate, itemExpiryDate, itemParentUnit, itemChildUnit, itemGST, code, formula, HSNCode, weight, otherInformation } = req.body;
        ...
        if (req.files && Array.isArray(req.files)) {
            try {
                const uploadResults = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer, "Epharma/items") // Folder name
                ));
        ...
        const processedOtherInfo = {};
        if (otherInformation) {
            const info = typeof otherInformation === 'string' ? JSON.parse(otherInformation) : otherInformation;
            if (info.keyFeatures)
                processedOtherInfo.keyFeatures = Array.isArray(info.keyFeatures) ? info.keyFeatures : [info.keyFeatures];
            if (info.benefits)
                processedOtherInfo.benefits = Array.isArray(info.benefits) ? info.benefits : [info.benefits];
            if (info.precautions)
                processedOtherInfo.precautions = Array.isArray(info.precautions) ? info.precautions : [info.precautions];
            if (info.allergyInfo)
                processedOtherInfo.allergyInfo = Array.isArray(info.allergyInfo) ? info.allergyInfo : [info.allergyInfo];
            if (info.sideEffects)
                processedOtherInfo.sideEffects = Array.isArray(info.sideEffects) ? info.sideEffects : [info.sideEffects];
            if (info.howToUse)
                processedOtherInfo.howToUse = String(info.howToUse).trim();
            if (info.safetyAdvice)
                processedOtherInfo.safetyAdvice = Array.isArray(info.safetyAdvice) ? info.safetyAdvice : [info.safetyAdvice];
            if (info.ingredients)
                processedOtherInfo.ingredients = Array.isArray(info.ingredients) ? info.ingredients : [info.ingredients];
        }
        // === MRP VERIFICATION (REAL-TIME) ===
        let mrpVerificationData = { status: 'pending', needsAdminReview: true };
        try {
    ...
    static createPremiumItem = catchAsyncErrors(async (req, res, next) => {
        const { itemName, itemInitialPrice, itemDescription, itemCategory, itemMfgDate, itemExpiryDate, itemParentUnit, itemChildUnit, itemGST, code, HSNCode, weight, otherInformation } = req.body;
        console.log("Requested to body : ", req.body);
        const fields = {
            itemName,
            itemInitialPrice,
    ...
    static updateItem = catchAsyncErrors(async (req, res, next) => {
        ...
        let imageUrls = existingItem.itemImages || [];
        ...
        const updatedItem = await ItemModel.findByIdAndUpdate(itemId, {
            ...updateData,
            itemImages: imageUrls,
            itemFinalPrice,
    ...
    static deleteItem = catchAsyncErrors(async (req, res, next) => {
        const itemId = req.params.itemId;
        const existingItem = await ItemModel.findById(itemId);
        if (!existingItem) {
            return next(new ApiError(404, "Item not found"));
        ...
        if (existingItem.itemImages && existingItem.itemImages.length > 0) {
            try {
                const publicIds = existingItem.itemImages.map((url) => {
    ...
    static getAllItems = catchAsyncErrors(async (req, res, next) => {
    ...
    static getItemsByCategory = catchAsyncErrors(async (req, res, next) => {
        ...
        const items = await ItemModel.find({ itemCategory: categoryId })
            .skip((page - 1) * limit)
            .limit(limit);
        if (items.length === 0) {
            return next(new ApiError(404, "No items found"));
    ...
    static getDealsOfTheDay = catchAsyncErrors(async (req, res, next) => {
        ...
        const formattedDeals = deals.map((deal) => {
            const gstRate = deal.itemGST?.gstRate ?? 0;
    ...
    static addToRecentlyViewedItems = catchAsyncErrors(async (req, res, next) => {
    ...
    static getRecentlyViewedItems = catchAsyncErrors(async (req, res, next) => {
        const userId = req.user?._id;
        if (!userId) {
    ...
    static getDynamicFeed = catchAsyncErrors(async (req, res, next) => {
        const userId = req.user?._id;
        if (!userId) {
            return next(new ApiError(401, "User not authenticated"));
        }
        ...
        if (queueLength < limit) {
            ...
            const allIds = new Set([
                ...personalizedItems.map((i) => i._id.toString()),
                ...trendingItems.map((i) => i._id.toString()),
                ...newItems.map((i) => i._id.toString())
            ]);
    ...
    static getAITrendingProducts = catchAsyncErrors(async (req, res, next) => {
        ...
        const scoredItems = globalCandidates.map(item => {
            ...
            if (userProfile) {
                if (userProfile.viewedCategories?.some((c) => c.toString() === item.itemCategory?.toString())) {
                    score += 30;
                }
            }
            ...
            if ((item.views || 0) > 100)
                score += 15;
            if ((item.itemDiscount || 0) > 20)
                score += 10;
            // C. Product Quality (Weight: 15%)
            if ((item.itemRatings || 0) >= 4.5)
                score += 15;
            ...
            if (isWinter && (nameLower.includes("code") || nameLower.includes("syrup")))
                score += 20; // Example keywords
        ...
        if (scoredItems.length > 5) {
            ...
            if (rand > 0.5)
                [scoredItems[0], scoredItems[1]] = [scoredItems[1], scoredItems[0]];
        }
    ...
    static getItemDetails = catchAsyncErrors(async (req, res, next) => {
        const { itemId } = req.params;
        if (!itemId) {
            return next(new ApiError(400, "Item ID is required"));
        }
