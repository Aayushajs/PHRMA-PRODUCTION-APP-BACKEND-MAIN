...
export default class NotificationLogService {
    static getActiveLogs = catchAsyncErrors(async (req, res, next) => {
        const { page = 1, limit = 20, type, userId, startDate, endDate, sortBy = "sentAt", order = "desc", isRead, } = req.query;
        ...
        try {
            ...
            if (cachedData) {
                return handleResponse(req, res, 200, "Active notification logs fetched from cache", cachedData);
            }
            ...
            if (startDate || endDate) {
                matchStage.sentAt = {};
                if (startDate)
                    matchStage.sentAt.$gte = new Date(startDate);
                if (endDate)
                    matchStage.sentAt.$lte = new Date(endDate);
            }
            ...
            return handleResponse(req, res, 200, "Active notification logs fetched successfully", responseData);
        }
        catch (error) {
    ...
    static getUserLogs = catchAsyncErrors(async (req, res, next) => {
        const { userId } = req.params;
        const { page = 1, limit = 20, type, isRead, startDate, endDate, sortBy = "sentAt", order = "desc", } = req.query;
        if (!userId || !mongoose.isValidObjectId(userId)) {
        ...
        try {
            const cachedData = await getCache(cacheKey);
            if (cachedData) {
                return handleResponse(req, res, 200, "User notification logs fetched from cache", cachedData);
            }
            const matchStage = {
                userId: userId,
                status: "SENT",
            ...
            await setCache(cacheKey, responseData, CACHE_TTL);
            return handleResponse(req, res, 200, "User notification logs fetched successfully", responseData);
        }
        catch (error) {
    ...
    static getLogById = catchAsyncErrors(async (req, res, next) => {
        ...
        try {
            const cachedData = await getCache(cacheKey);
            if (cachedData) {
                return handleResponse(req, res, 200, "Notification log fetched from cache", cachedData);
            }
            const pipeline = [
            ...
            return handleResponse(req, res, 200, "Notification log fetched successfully", log);
        }
        catch (error) {
    ...
    static markAsRead = catchAsyncErrors(async (req, res, next) => {
        const { id } = req.params;
        ...
        try {
            ...
            if (!updatedLog) {
                return next(new ApiError(404, "Notification log not found or unauthorized"));
            }
            ...
            return handleResponse(req, res, 200, "Notification marked as read", updatedLog);
        }
        catch (error) {
    ...
    static getUserStats = catchAsyncErrors(async (req, res, next) => {
        const { userId } = req.params;
        ...
        try {
            const cachedData = await getCache(cacheKey);
            if (cachedData) {
                return handleResponse(req, res, 200, "User notification stats fetched from cache", cachedData);
            }
            const pipeline = [
                {
                    $match: {
            ...
            return handleResponse(req, res, 200, "User notification statistics fetched successfully", responseData);
        }
        catch (error) {
