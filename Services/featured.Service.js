...
export default class FeaturedMedicineService {
    //CREATE-----------------------------------------------------------------
    static createFeaturedMedicine = catchAsyncErrors(async (req, res, next) => {
        const { title, remarks, description = "", category, discount = 0, stock, featured = false, ratings = 0, createdBy, } = req.body;
        const createdById = req.user?._id ?? createdBy;
        if (req.file) {
        ...
        if (req.file) {
            try {
                const uploadResult = await uploadToCloudinary(req.file.buffer, "Epharma/medicines");
            ...
            return next(new ApiError(400, "Either upload an image file or provide imageUrl"));
        }
        ...
        if (existingMedicine) {
            return next(new ApiError(409, "Featured medicine with same title already exists"));
        }
        const cleanData = {
            title: title.trim(),
        ...
        return handleResponse(req, res, 201, "Featured medicine created successfully", newMedicine);
    });
    // ALL GET -----------------------------------------------------
    static getFeaturedMedicines = catchAsyncErrors(async (req, res, next) => {
        try {
            const cachedData = await getCache(CACHE_KEY);
            if (cachedData) {
                return handleResponse(req, res, 200, "Data fetched from Redis Cache", cachedData);
            }
            ...
            return handleResponse(req, res, 200, " Data fetched from MongoDB", payload);
        }
        catch (error) {
    ...
    //UPDATE ------------------------------------------------------
    static updateFeaturedMedicine = catchAsyncErrors(async (req, res, next) => {
        const { id } = req.params;
        ...
        const updatedMedicine = await FeaturedMedicine.findByIdAndUpdate(id, updates, {
            new: true,
            runValidators: true,
        });
        if (!updatedMedicine)
        ...
        process.nextTick(async () => {
            try {
                ...
                if (!users || users.length === 0)
                    return;
                const titleText = updatedMedicine.title || "An item";
        ...
        return handleResponse(req, res, 200, "Medicine updated successfully", updatedMedicine);
    });
    // delete ------------------------------------------------------
    static deleteFeaturedMedicine = catchAsyncErrors(async (req, res, next) => {
        const { id } = req.params;
        if (!id)
            return next(new ApiError(400, "Invalid medicine ID"));
        ...
        await deleteCache(CACHE_KEY).catch(() => null);
        return handleResponse(req, res, 200, "Medicine deleted successfully", deletedMedicine);
    });
}
export class FeaturedMedicineLogService {
    static CACHE_PREFIX = "featuredLogs";
    static CACHE_TTL = 1800;
    static getAllLogs = catchAsyncErrors(async (req, res, next) => {
        const { page = 1, limit = 20, search = "", action, userId, medicineId, startDate, endDate, sortBy = "createdAt", order = "desc" } = req.query;
