...
const { CACHE_PREFIX, DEFAULT_LIMIT, MAX_LIMIT, LIST_CACHE_TTL, CLOUDINARY_FOLDERS, } = CATEGORY_CONSTANTS;
const buildListCacheKey = (query) => `${CACHE_PREFIX}:list:${crypto
    .createHash("md5")
    .update(JSON.stringify(query))
    .digest("hex")}`;
const genCodeFromName = (name) => name
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .slice(0, 60);
export default class CategoryService {
    static async createCategory(req, res, next) {
        try {
            const { name, title, description, code, offerText, priority = 0, isFeatured = false, isActive = true, } = req.body;
            ...
            process.nextTick(async () => {
                try {
                    ...
                    const [imageResults, bannerResults] = await Promise.all([
                        Promise.all(imageFiles.map((f) => uploadToCloudinary(f.buffer, CLOUDINARY_FOLDERS.IMAGES))),
                        bannerFiles.length
                            ? Promise.all(bannerFiles.map((f) => uploadToCloudinary(f.buffer, CLOUDINARY_FOLDERS.BANNERS)))
                            : [],
                    ]);
                    ...
                    const users = await User.find({ fcmToken: { $ne: null } }).select("_id name fcmToken");
                    const notificationTitle = "New Category Added!";
                    const body = `${category.title} has been added to the store.`;
                    await NotificationService.sendNotificationToMultipleUsers(users.filter(u => u.fcmToken).map(u => ({
                        _id: u._id.toString(),
                        fcmToken: u.fcmToken,
                        name: u.name
    ...
    static async getAllCategory(req, res, next) {
        try {
            const { page = "1", limit = String(DEFAULT_LIMIT), sortBy = "priority", order = "desc", isActive, isFeatured, } = req.query;
            ...
            if (cachedData) {
                return handleResponse(req, res, 200, "Categories fetched from Redis cache", cachedData);
            }
            ...
            return handleResponse(req, res, 200, "Categories fetched from MongoDB", payload);
        }
        catch (error) {
            console.error("Error retrieving categories:", error);
            return next(new ApiError(500, `Failed to retrieve categories: ${error.message}`));
        }
    }
    static async getCategoriesSimple(req, res, next) {
        try {
            ...
            if (!userId && req.headers.authorization) {
                try {
                    ...
                    if (token) {
                        const decoded = jwtTokens.verify(token, process.env.USER_SECRET_KEY);
                        userId = decoded._id;
                    }
                }
                catch (e) {
            ...
            let matchStage = { isActive: isActive === "true" };
            ...
            const cachedData = await getCache(cacheKey);
            if (cachedData) {
                return handleResponse(req, res, 200, "Categories retrieved from cache", cachedData);
            }
            ...
            const pipeline = [
                ...
                {
                    $sort: {
                        isRecentlyViewed: -1, // Viewed first
                        priority: -1,
                        createdAt: -1,
                    },
                },
            ...
            return handleResponse(req, res, 200, "Categories retrieved successfully using aggregation", responseData);
        }
        catch (error) {
    ...
    static async getCategoryById(req, res, next) {
        try {
            ...
            const cachedData = await getCache(cacheKey);
            if (cachedData) {
                return handleResponse(req, res, 200, "Category retrieved from cache", cachedData);
            }
            const pipeline = [
                { $match: { _id: id } },
                {
            ...
            return handleResponse(req, res, 200, "Category retrieved successfully", category);
        }
        catch (error) {
            console.error("Get category by ID error:", error);
            return next(new ApiError(500, `Failed to retrieve category: ${error.message}`));
        }
    }
    static async updateCategory(req, res, next) {
        try {
            const { id } = req.params;
            const { name, title, description, code, offerText, priority, isFeatured, isActive, } = req.body;
            if (!id) {
                return next(new ApiError(400, "Category ID is required"));
            }
            ...
            if (name && name !== existingCategory.name) {
                ...
                if (conflictCategory) {
                    return next(new ApiError(400, "Category with this name or code already exists"));
                }
            }
            const updateData = {
                updatedBy: req.user?._id,
            ...
            const updatedCategory = await CategoryModel.findByIdAndUpdate(id, updateData, { new: true, runValidators: true });
            // Clear cache
            await Promise.all([
            ...
            process.nextTick(async () => {
                try {
                    ...
                    const updatedTitle = updatedCategory?.title || updatedCategory?.name || existingCategory.name;
                    ...
                    await NotificationService.sendNotificationToMultipleUsers(users.filter(u => u.fcmToken).map(u => ({
                        ...
                        payload: {
                            ...
                            imageUrl: updatedCategory?.imageUrl?.length > 0 ? updatedCategory?.imageUrl[0] : null
                        }
                    });
                }
                catch (err) {
            ...
            return handleResponse(req, res, 200, "Category updated successfully", updatedCategory);
        }
        catch (error) {
            console.error("Update category error:", error);
            return next(new ApiError(500, `Failed to update category: ${error.message}`));
        }
    }
    // Delete category (soft delete)
    static async ActiovationCategory(req, res, next) {
        try {
            const { id } = req.params;
            ...
            if (permanent === "true") {
                ...
                return handleResponse(req, res, 200, "Category deactivated successfully", updatedCategory);
            }
        }
        catch (error) {
            console.error("Delete category error:", error);
            return next(new ApiError(500, `Failed to delete category: ${error.message}`));
        }
    }
    // Bulk toggle active status
    static async bulkToggleActive(req, res, next) {
        try {
            ...
            if (existingCategories.length !== categoryIds.length) {
                ...
                return next(new ApiError(400, `Categories not found: ${missingIds.join(", ")}`));
            }
            const updateResult = await CategoryModel.updateMany({ _id: { $in: categoryIds } }, {
                isActive,
                updatedBy: req.user?._id,
                updatedAt: new Date(),
            });
            const updatedCategories = await CategoryModel.find({ _id: { $in: categoryIds } }, { _id: 1, name: 1, isActive: 1 });
            const cachePromises = [
                deleteCachePattern(`${CACHE_PREFIX}:list:*`),
                deleteCachePattern(`${CACHE_PREFIX}:simple:*`),
            ...
            return handleResponse(req, res, 200, `Successfully ${isActive ? "activated" : "deactivated"} ${updateResult.modifiedCount} categories`, {
            ...
            return next(new ApiError(500, `Failed to bulk toggle categories: ${error.message}`));
        }
    }
    static addToRecentlyViewedCategories = catchAsyncErrors(async (req, res, next) => {
    ...
    static getRecentlyViewedCategories = catchAsyncErrors(async (req, res, next) => {
        const userId = req.user?._id;
        if (!userId) {
            return next(new ApiError(401, "User not authenticated"));
        }
        ...
        const cachedData = await getCache(cacheKey);
        if (cachedData) {
            return handleResponse(req, res, 200, "Recently viewed categories fetched from cache", cachedData);
        }
        ...
        return handleResponse(req, res, 200, "Recently viewed categories fetched successfully", result);
    });
}
export class CategoryLogService {
    static CACHE_PREFIX = "categoryLogs";
    static CACHE_TTL = 1800;
    // Debug method to check database connection and data
    static getDebugInfo = catchAsyncErrors(async (req, res, next) => {
    ...
    static getAllLogs = catchAsyncErrors(async (req, res, next) => {
        const { page = 1, limit = 20, search = "", action, userId, categoryId, startDate, endDate, sortBy = "createdAt", order = "desc", } = req.query;
        const pageNum = parseInt(page) || 1;
        ...
        const cachedData = await getCache(cacheKey);
        if (cachedData) {
            return handleResponse(req, res, 200, "Category logs fetched from cache", cachedData);
        }
        const matchStage = {};
        if (search && search.toString().trim()) {
            const searchRegex = new RegExp(search.toString().trim(), "i");
        ...
        if (userId && mongoose.isValidObjectId(userId))
            matchStage.performedBy = new mongoose.Types.ObjectId(userId);
        ...
        const finalSortBy = sortBy === "createdAt" ? "timestamp" : sortBy;
        ...
        return handleResponse(req, res, 200, "Category logs fetched successfully", responseData);
    });
    static getLogById = catchAsyncErrors(async (req, res, next) => {
        const { id } = req.params;
        ...
        const cachedData = await getCache(cacheKey);
        if (cachedData) {
            return handleResponse(req, res, 200, "Category log fetched from cache", cachedData);
        }
        const pipeline = [
        ...
        return handleResponse(req, res, 200, "Category log fetched successfully", log);
    });
    static getLogsByDateRange = catchAsyncErrors(async (req, res, next) => {
    ...
    static getLogStats = catchAsyncErrors(async (req, res, next) => {
        ...
        const cachedData = await getCache(cacheKey);
        if (cachedData) {
            return handleResponse(req, res, 200, "Category log stats fetched from cache", cachedData);
        }
        const pipeline = [
            {
                $match: {
        ...
        return handleResponse(req, res, 200, "Category log statistics fetched successfully", responseData);
    });
}
