...
function extractMedicinesWithRegex(text: string): MedicineDetails[] {
...
function extractMedicinesFallback(text: string): MedicineDetails[] {
  const medicines: MedicineDetails[] = [];
  ...
  const isPrescriptionSection = (lineText: string) =>
    prescriptionKeywords.some((kw) => lineText.includes(kw));
  ...
  for (const line of lines) {
    ...
    if (
      trimmed.match(/^(Parameter|Value|Status|Medicine|Dosage|Instructions)$/i)
    ) {
      continue;
    }
    ...
    if (inPrescriptionSection && trimmed.length > 0) {
      ...
      const medicineMatch = trimmed.match(
        /^([A-Za-z]+(?:\s+[A-Za-z]+)?)\s+(\d+\s*(?:days?|weeks?|months?))\s+([\d\s]+(?:for|x|times?)\s+[\d]+)$/i
      );
...
export async function processPrescriptionBuffer(
  buffer: Buffer
): Promise<OcrResult> {
  await ensureTempDir();
