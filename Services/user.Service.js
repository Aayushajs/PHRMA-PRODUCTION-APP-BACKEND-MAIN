...
export default class UserService {
    static signup = catchAsyncErrors(async (req, res, next) => {
        const { name, email, password, phone, age, dob, role, fcmToken, address } = req.body;
        ...
        if (existingUser) {
            ...
            if (existingUser.phone === phone) {
                return next(new ApiError(400, "User with this phone number already exists"));
            }
        }
        ...
        if (req.file) {
            try {
                const result = await uploadToCloudinary(req.file.buffer, "Epharma/profiles");
                userData.ProfileImage = [result.secure_url];
            }
            catch (error) {
                return next(new ApiError(500, `Profile image upload failed: ${error.message || error}`));
            }
        }
    ...
    static verifyOtp = catchAsyncErrors(async (req, res, next) => {
    ...
    static googleAuthLogin = catchAsyncErrors(async (req, res, next) => {
    ...
    static getUserProfile = catchAsyncErrors(async (req, res, next) => {
        ...
        return handleResponse(req, res, 200, "User profile fetched successfully", formattedUser);
    });
    static updateUserProfile = catchAsyncErrors(async (req, res, next) => {
        const userId = req.user?._id;
        const requestBody = req.body || {};
        const { name, email, address, phone, age, dob, ProfileImage, fcmToken } = requestBody;
        if (!userId) {
        ...
        if (req.file) {
            try {
                const uploadResult = await uploadToCloudinary(req.file.buffer, `Epharma/profiles/${userId}`);
                user.ProfileImage = [uploadResult.secure_url];
            }
            catch (error) {
                return next(new ApiError(500, `Failed to upload image: ${error.message || error}`));
            }
        }
        ...
        if (address && typeof address === 'object') {
            try {
                ...
                if (addressData.zip !== undefined)
                    user.address.zip = addressData.zip || "";
                if (addressData.country !== undefined)
                    user.address.country = addressData.country || "India";
        ...
        return handleResponse(req, res, 200, "User profile updated successfully", userResponse);
    });
}
