...
export default class AdvertisementService {
    static CACHE_PREFIX = "advertisements";
    ...
    static createAd = catchAsyncErrors(async (req, res, next) => {
        const { title, description, type, brand, itemId, categoryId, offerText, startDate, endDate, isActive = true } = req.body;
        // Validation
        if (!title?.trim()
            || !description?.trim()
            || !type
            || !startDate
            || !endDate) {
            return next(new ApiError(400, "Title, description, type, startDate, and endDate are required"));
        }
        ...
        return handleResponse(req, res, 201, "Advertisement created successfully", {
            advertisement: {
    ...
    static updateAd = catchAsyncErrors(async (req, res, next) => {
        const { adId } = req.params;
        const { title, description, type, brand, itemId, categoryId, offerText, startDate, endDate, isActive } = req.body;
        ...
        const updatedAd = await Advertisement.findByIdAndUpdate(adId, updateData, {
            new: true,
            runValidators: true
        });
        if (!updatedAd) {
        ...
        return handleResponse(req, res, 200, "Advertisement updated successfully", {
            advertisement: {
                _id: updatedAd._id,
    ...
    static getCurrentlyRunningAds = catchAsyncErrors(async (req, res, next) => {
        ...
        try {
            ...
            if (cachedData) {
                return handleResponse(req, res, 200, "Currently running advertisements fetched from Redis Cache", cachedData);
            }
            ...
            return handleResponse(req, res, 200, "Currently running advertisements fetched from MongoDB", responseData);
        }
        catch (error) {
    ...
    static getActiveAds = catchAsyncErrors(async (req, res, next) => {
        const { page = 1, limit = 10, type, categoryId, search = "", sortBy = "createdAt", order = "desc" } = req.query;
        ...
        if (type && ["Product", "Brand", "Offer", "Event"].includes(type)) {
    ...
    static trackClick = catchAsyncErrors(async (req, res, next) => {
        const { adId } = req.params;
        ...
        const recentClick = advertisement.adClickTracking.find(click => click.userId.toString() === userId.toString() &&
        ...
        if (recentClick) {
            ...
            return handleResponse(req, res, 200, `Click already tracked. Next click allowed in ${hoursLeft} hours`, {
                adId,
                userId,
        ...
        process.nextTick(async () => {
            try {
                ...
                await NotificationService.sendNotificationToMultipleUsers(recipients.filter(r => r.fcmToken).map(r => ({
                    _id: r._id.toString(),
                    fcmToken: r.fcmToken,
                    name: r.name
                })), title, body, {
        ...
        return handleResponse(req, res, 200, "Click tracked successfully", {
            adId,
            userId,
    ...
    static getAnalytics = catchAsyncErrors(async (req, res, next) => {
        const { period = "7d", adId, type, startDate, endDate } = req.query;
        let dateFilter = {};
        const currentDate = new Date();
        ...
        if (adId && mongoose.isValidObjectId(adId)) {
            matchStage._id = new mongoose.Types.ObjectId(adId);
        }
        if (type && ["Product", "Brand", "Offer", "Event"].includes(type)) {
            matchStage.type = type;
        }
    ...
    static deleteAd = catchAsyncErrors(async (req, res, next) => {
        const { adId } = req.params;
        if (!adId || !mongoose.isValidObjectId(adId)) {
            return next(new ApiError(400, "Invalid advertisement ID"));
        }
        ...
        return handleResponse(req, res, 200, "Advertisement deleted successfully", {
    ...
    static softDeleteAd = catchAsyncErrors(async (req, res, next) => {
        const { adId } = req.params;
        if (!adId || !mongoose.isValidObjectId(adId)) {
            return next(new ApiError(400, "Invalid advertisement ID"));
        }
        ...
        if (!existingAd.isActive) {
            return handleResponse(req, res, 200, "Advertisement already deactivated", {
                advertisement: {
                    _id: existingAd._id,
                    title: existingAd.title,
                    isActive: existingAd.isActive,
        ...
        const updatedAd = await Advertisement.findByIdAndUpdate(adId, {
            isActive: false,
            updatedBy: req.user?.id || null,
            updatedAt: new Date()
        ...
        return handleResponse(req, res, 200, "Advertisement deactivated successfully", {
            advertisement: {
                _id: updatedAd._id,
                title: updatedAd.title,
                description: updatedAd.description,
...
export class AdvertisementLogService {
    static CACHE_PREFIX = "advertisementLogs";
    static CACHE_TTL = 1800; // 30 minutes
    static getAllLogs = catchAsyncErrors(async (req, res, next) => {
        const { page = 1, limit = 20, search = "", action, userId, advertisementId, startDate, endDate, sortBy = "timestamp", order = "desc" } = req.query;
        const pageNum = parseInt(page) || 1;
        ...
        try {
            ...
            if (advertisementId && mongoose.isValidObjectId(advertisementId)) {
                matchStage.advertisementId = new mongoose.Types.ObjectId(advertisementId);
            }
            if (startDate || endDate) {
    ...
    static getLogsByAdvertisement = catchAsyncErrors(async (req, res, next) => {
