...
export function extractMedicinesWithRegex(text: string): MedicineDetails[] {
  ...
  const tablePattern = /([A-Za-z][A-Za-z0-9-]*(?:\s+[A-Za-z0-9-]+)?)\s+(\d+\s*(?:days?|weeks?|months?))\s+([\d\s]+(?:for|x|times?)\s+[\d]+)/gi;
  ...
  if (medicines.length === 0) {
    const standardPattern =
      /([A-Za-z][A-Za-z0-9- ]{1,30}?)\s+(\d+\.?\d*\s*(?:mg|ml|units?))\s+((?:\d+\s*x\s*)?(?:once|twice|thrice|daily|twice daily|once daily))\s+(?:for\s+)?(\d+\s*(?:days?|weeks?|months?))/gi;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inPrescriptionSection) {
      ...
      if (i + 2 < lines.length) {
        ...
        if (isDuration && isFreq && /^[A-Za-z][A-Za-z0-9\s-]{1,40}$/.test(possibleName)) {
          medicines.push({
      ...
      if (dosageMatch) {
        const drugName = trimmed
          .slice(0, dosageMatch.index ?? 0)
          .replace(/[\s,;:-]+$/g, "")
          .trim();
        if (drugName) {
          medicines.push({
