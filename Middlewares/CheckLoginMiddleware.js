...
export const customersMiddleware = catchAsyncErrors(async (req, res, next) => {
    ...
    const decoded = jwt.verify(headerToken, process.env.USER_SECRET_KEY);
...
export const adminMiddleware = catchAsyncErrors(async (req, res, next) => {
...
export const userMiddleware = catchAsyncErrors(async (req, res, next) => {
    const headerToken = req.headers.authorization?.split(" ")[1];
    const token = req.cookies.userToken || headerToken;
    if (!token) {
        return next(new ApiError(401, "Unauthorized: No token provided"));
...
export const authenticatedUserMiddleware = catchAsyncErrors(async (req, res, next) => {
    const headerToken = req.headers.authorization?.split(" ")[1];
    const token = req.cookies.userToken || headerToken;
    if (!token) {
        return next(new ApiError(401, "Unauthorized: No token provided"));
